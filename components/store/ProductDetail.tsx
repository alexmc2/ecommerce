...
export default function ProductDetail({ product }: ProductDetailProps) {
  ...
  const discountPercentage = hasCompare && product.compareAtPrice
    ? Math.max(
        1,
        Math.round(
    ...
    : null;
  const savings = hasCompare && product.compareAtPrice
  ...
  const descriptionHasContent = Boolean(product.body && product.body.length > 0);
    ...
    <section className="flex flex-col gap-16">
      ...
      {descriptionHasContent && (
        <div className="mx-auto flex w-full max-w-4xl flex-col gap-6 rounded-[34px] border border-border/60 bg-background p-8">
          ...
          <PortableText value={product.body ?? []} components={portableComponents} />
        </div>
      )}

      <div className="rounded-[32px] bg-primary/5 p-6 text-sm text-muted-foreground sm:p-8">
        <p>
          Need help deciding? Contact us for personalised recommendations or to
