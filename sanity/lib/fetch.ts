...
const buildImage = (image?: SanityImageWithAsset | null): ProductImage | null => {
  if (!image?.asset?._id) {
    return null;
  }
  ...
  const url = (isSvg ? builder : builder.width(800).height(800).fit("crop")).url();
...
const mapOptions = (options?: ProductOptionRaw[]): ProductOption[] | undefined => {
  if (!options?.length) {
    return undefined;
  }

  const cleaned = options
    .filter((option): option is Required<ProductOptionRaw> => Boolean(option?.name && option?.value))
    .map((option) => ({
...
const mapVariants = (variants?: ProductVariantRaw[]): ProductVariant[] | undefined => {
  if (!variants?.length) {
    return undefined;
  }

  const mapped = variants
    .filter((variant): variant is ProductVariantRaw & { title: string; sku: string } =>
...
const mapProductListItem = (product?: ProductCardRaw | null): ProductListItem | null => {
  if (!product) {
    return null;
  }
...
const mapProductDetail = (product?: ProductDetailRaw | null): ProductDetail | null => {
  if (!product) {
    return null;
  }
  ...
  const gallery = product.images?.map(buildImage).filter(Boolean) as ProductImage[];
...
export const fetchSanityProducts = async ({
  ...
  if (collectionId) {
    ...
    const collectionData = data as { products?: ProductCardRaw[] | null } | null;
  ...
  if (ids && ids.length) {
    ...
    const mapped = mapProductCollection(data as ProductCardRaw[] | null | undefined);
